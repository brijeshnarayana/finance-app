// FinBizz - Main Application Entry Point
...
import {
  ...
  sicDivisions,
  isicSections,
} from "./data/sectors-data.js";
import { businessTermsLesson } from "./data/business-terms-lesson.js";
import { financeTermsLesson } from "./data/finance-terms-lesson.js";

// Append glossary terms
financeCurriculum.push(financeTermsLesson);
businessCurriculum.push(businessTermsLesson);
...
function renderDashboard() {
  ...
  document.getElementById("topbar-breadcrumb").textContent =
    Dates.getGreeting() + "! Ready to learn?";
          ...
          <div class="card card-glow-${meta.color} animate-slide-up">
            ...
            <div class="flex flex-col gap-2">
              ...
              `,
                      ...
                      .join("")
                  : '<div class="text-muted text-sm" style="padding:12px">All caught up! 🎉</div>'
              }
            </div>
          </div>
        `;
...
window.toggleTask = function (taskId, lessonId) {
  ...
  if (task) {
    ...
    if (task.completed) {
      Storage.setLessonProgress(lessonId, {
        completed: true,
...
function renderLesson(lessonId) {
  ...
  if (!lesson) {
    document.getElementById("app-content").innerHTML =
      '<div class="empty-state"><div class="empty-state-icon">📚</div><div class="empty-state-title">Lesson not found</div></div>';
    return;
  }
        ...
        <div class="card card-lg">
          ${c.actionItems
            .map((item, i) => {
              const aiKey = `ai_${i}`;
              const checked =
                progress.actionItems && progress.actionItems[aiKey];
              return `
...
window.completeLesson = function (lessonId) {
  ...
  if (newState) {
    ...
    Notifications.sendLessonComplete(
      document.querySelector(".lesson-title")?.textContent || "Lesson",
    );
  } else {
    showToast("Lesson marked as incomplete", "info");
    Storage.unlogActivity(Dates.today());
    Scheduler.updateGoalProgress(curricula, Dates.today());
  }
  updateNavBadges();
  renderLesson(lessonId);
...
window.analyzeNotes = async function (lessonId) {
  ...
  if (!notes || notes.length < 20) {
    showToast(
      "Please write at least a few sentences in your notes before analyzing.",
      "warning",
    );
    return;
  }
  ...
  container.innerHTML =
    '<div class="ai-response-panel" style="text-align:center;padding:32px"><div class="loading-spinner" style="margin:0 auto"></div><div class="text-muted text-sm mt-4">AI is analyzing your notes...</div></div>';
  ...
  content.innerHTML = `
    ...
    <div id="news-grid" class="grid grid-auto stagger">
      ${Array(6)
        ...
        .map(
          () =>
            '<div class="card" style="height:200px"><div class="skeleton" style="height:100%;width:100%"></div></div>',
        )
        .join("")}
    </div>
  `;
...
function renderNewsGrid(articles) {
  ...
  grid.innerHTML = articles
    .map((article) => {
      ...
      const timeAgo = Dates.getRelativeTime(
        new Date(article.pubDate).getTime(),
      );
      const cats = (article.category || [])
        .map((c) => `<span class="tag" style="pointer-events:none">${c}</span>`)
        .join("");
      const isStartupRelated = (article.keywords || []).some((k) =>
        ["startup", "investment", "funding"].includes(k.toLowerCase()),
      );
...
async function renderMarket() {
  ...
  document.getElementById("topbar-breadcrumb").textContent =
    "Daily Market Intelligence & SIP Suggestions";
    ...
    <div id="market-content">
      <div class="grid grid-auto stagger">
        ${Array(8)
          ...
          .map(
            () =>
              '<div class="card" style="height:120px"><div class="skeleton" style="height:100%;width:100%"></div></div>',
          )
          .join("")}
      </div>
    </div>
...
window.showMarketTab = async function (tab) {
  document
    .querySelectorAll("#market-tabs .tab")
    .forEach((t) => t.classList.remove("active"));
  event?.target?.classList.add("active");
  ...
  if (tab === "watchlist") {
    ...
    container.innerHTML = `
      ...
      <div class="grid grid-3 mb-6 stagger">
        ${Object.entries(sipWatchlist.sipStrategies)
          .map(
            ([key, strat]) => `
      ...
      ${["blueChips", "growth", "dividend", "etfs"]
        .map((cat) => {
          const names = {
            blueChips: "🏛️ Blue Chips",
            growth: "🚀 Growth",
            dividend: "💵 Dividend",
            etfs: "📦 ETFs",
          };
...
function renderTracker() {
  ...
  if (weekReorg) {
    Notifications.sendWeeklyReorg();
    showToast("Weekly goals reorganized!", "info");
  }
  if (monthReorg) {
    Notifications.sendMonthlyReorg();
    showToast("Monthly goals reorganized!", "info");
  }

  const weeklyGoals =
    Storage.getWeeklyGoals(weekKey) ||
    Scheduler.generateWeeklyGoals(curricula, today);
  const monthlyGoals =
    Storage.getMonthlyGoals(monthKey) ||
    Scheduler.generateMonthlyGoals(curricula, today);
  ...
  window.showTrackerView(
    "daily",
    document.querySelector(".tracker-tab.active"),
  );
}

window.showTrackerView = function (view, btn) {
  document
    .querySelectorAll(".tracker-tab")
    .forEach((t) => t.classList.remove("active"));
  ...
  if (view === "daily") {
    ...
    container.innerHTML = `
      ...
      <div class="grid grid-2 mb-6 stagger">
        ${Object.entries(weeklyGoals.categories)
          .filter(([k]) => k !== "general")
          .map(([cat, data]) => {
            const meta = categoryMeta[cat];
            const pct = data.targetLessons
              ? Math.round((data.completedLessons / data.targetLessons) * 100)
              : 0;
            return `
            <div class="card card-glow-${meta.color}">
      ...
      ${
        ...
        <div class="card animate-slide-up">
          ...
          ${weeklyGoals.categories.general.milestones
            .map(
              (m) => `
    ...
    container.innerHTML = `
      ...
      <div class="grid grid-3 mb-6 stagger">
        ${Object.entries(monthlyGoals.categories)
          .map(([cat, data]) => {
            const meta = categoryMeta[cat];
            const pct = data.targetLessons
              ? Math.round((data.completedLessons / data.targetLessons) * 100)
...
function renderSettings() {
  ...
  content.innerHTML = `
    <div style="max-width:700px" class="animate-fade-in">
      ...
      <!-- About -->
      <div class="card">
        <div class="font-bold text-lg mb-4">ℹ️ About FinBizz</div>
        <div class="text-sm text-muted">
          <p>FinBizz — A comprehensive daily learning & growth platform for Finance, Business, Stock Market, AI News, and Investment Analysis.</p>
...
window.exportData = function () {
  ...
  a.href = url;
  a.download = `finbizz-backup-${Dates.today()}.json`;
  a.click();
...
function renderSectors() {
  ...
  document.getElementById("topbar-breadcrumb").textContent =
    "Classification Systems & Sector Analysis";
          ...
          <div class="card card-sm">
            ...
            <div class="flex flex-wrap gap-1 mt-3">
              ${sys.useCases
                ...
                .map(
                  (u) =>
                    `<span class="tag" style="pointer-events:none;font-size:0.7rem">${u}</span>`,
                )
                .join("")}
            </div>
          </div>
      ...
      <div class="sector-grid stagger">
        ${gicsSectors
          ...
          <a class="sector-card" href="#sector/${sector.id}" style="text-decoration:none;color:inherit">
            ...
            <div class="flex flex-wrap gap-1 mt-3">
              ${sector.growthDrivers
                .slice(0, 2)
                .map(
                  (d) =>
                    `<span class="tag" style="pointer-events:none;font-size:0.65rem;border-color:${sector.color}30;color:${sector.color}">${d.split(" ").slice(0, 3).join(" ")}</span>`,
                )
                .join("")}
            </div>
          </a>
...
function renderSectorDetail(sectorId) {
  ...
  if (!sector) {
    document.getElementById("app-content").innerHTML =
      '<div class="empty-state"><div class="empty-state-icon">🏢</div><div class="empty-state-title">Sector not found</div><div class="empty-state-desc"><a href="#sectors">Back to Sectors</a></div></div>';
    return;
  }

  document.getElementById("topbar-title").textContent = sector.name;
  document.getElementById("topbar-breadcrumb").textContent =
    `Sectors & Industries → GICS ${sector.gicsCode}`;
  ...
  window.showSectorTab(
    "overview",
    sectorId,
    document.querySelector(".sector-sub-tab.active"),
  );
}
...
window.showSectorTab = function (tab, sectorId, btn) {
  ...
  document
    .querySelectorAll(".sector-sub-tab")
    .forEach((t) => t.classList.remove("active"));
  if (btn) btn.classList.add("active");
  ...
  if (tab === "overview") {
    container.innerHTML = `
      <div class="stagger">
        ...
        <div class="card mb-6">
          ...
          <div class="flex flex-col gap-2">
            ${sector.overview.keyCharacteristics
              .map(
                (c) => `
    ...
    const steps = [fw.step1, fw.step2, fw.step3, fw.step4, fw.step5].filter(
      Boolean,
    );
    container.innerHTML = `
      <div class="stagger">
...
function init() {
  ...
  try {
    // Update date display
    document.getElementById("topbar-date").textContent = Dates.format(
      Dates.today(),
      "medium",
    );
    ...
    console.log("FinBizz initialized successfully ✓");
  } catch (err) {
